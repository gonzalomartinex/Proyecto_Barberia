        ...
        for barbero in barberos:
            for servicio in servicios:
                ...
                while hora_actual <= hora_limite:
                    aware_dt = timezone.make_aware(hora_actual, timezone.get_current_timezone())
                    if not Turno.objects.filter(barbero=barbero, servicio=servicio, fecha_hora=aware_dt).exists():
